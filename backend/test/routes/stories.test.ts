...
describe('GET /api/reader/stories', () => {
  ...
  it('returns top story ids', async () => {
    globalThis.fetch = vi.fn(async (url: string | URL | Request) => {
      ...
      if (path.endsWith('/topstories.json')) return new Response(JSON.stringify([7, 8]), { status: 200 });
  ...
  it('maps latest to newstories', async () => {
    const spy = vi.fn(async (_url: unknown) => new Response(JSON.stringify([1]), { status: 200 }));
